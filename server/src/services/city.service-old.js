...
async function collectResource(cityId, buildingId) {
    ...
    try {
        ...
        // Manual collection is instant and uncapped
        await client.query(
            `
            UPDATE city_resources
...
// Population = Cottage capacity + base population starting bonus
...
// Continuous gold accrual and worker calculation: (Cottage Population * 0.6) + 200 Base Workers
function computeLiveResources(resources, buildings) {
    ...
    // (cottagesPopulation * 0.6) + 200 flat base workers
...
async function getFullCityState(cityId) {
    ...
    // 2. Fetch base components
    const city = await getCity(cityId);
    const rawResources = await getResources(cityId);
    const buildings = await getBuildings(cityId);

    // 3. Compute continuous live resources (gold accrual)
    ...
    // 5. Compute Warehouse Protected vs At-Risk amounts
    const totalStored = 
